...
const TablePagination = React.forwardRef(
  (
    { page, total, perpage, onChange, ...rest }: TablePaginationProps,
    ref: React.Ref<any>,
  ) => {
    let [{ perpages, color, variant, slotProps, ...props }] =
      useThemeComponent<any>("TablePagination", rest, {});
    color ??= "default";
    variant ??= "fill";
    perpages ??= [30, 50, 100];
    ...
    return (
      ...
      >
        {isPerpage && (
          <Tag
            baseClass="table-pagination-perpage"
            flexBox
            flexRow
            gap={1}
            alignItems="center"
