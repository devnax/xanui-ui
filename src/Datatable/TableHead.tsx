  ...
  return (
    <TableHead position="relative">
      <TableRow bgcolor="paper.primary" borderBottom={1}>
        {!hideCheckbox && (
          <TableCell th width={40}>
            <Checkbox
              size={compact ? "sm" : "md"}
              checkIcon={
                state.selected.length && !state.selectAll ? (
                  <IntermidiatIcon />
                ) : undefined
              }
              checked={checked}
              onChange={() => {
                ...
                for (let i = 0; i < rows.length; i++) {
                  const row = rows[i];
                  const isDisable =
                    (disableRow ? disableRow(row, state) : false) || false;
                  if (!isDisable && row.id) {
                    ids.push(row.id);
                  } else {
                    undefinedCount += 1;
        ...
        {columns.map(({ label, field: _f, sortable, ...rest }, idx) => (
          <TableCell key={idx} th textAlign="left" {...rest}>
            <Stack
        ...
        {!!(
          rows.length &&
          rowAction &&
          rowAction((rows as any)[0])?.length
