...
| `color`           | `UseColorTemplateColor` (except `default`) | `'primary'`     | Active color applied to the track when checked.              |
...
export default function CustomSwitch() {
  return (
    <Switch
      ...
      slotProps={{
        track: { bgcolor: 'success.surface', border: '1px solid', borderColor: 'success.main' },
        thumb: { shadow: 1 },
      }}
      checked
    />
